...
- **[HIGH] Generic content create/edit form hangs server-side for ALL content types.** `app/admin/content/[type]/new/page.tsx` and `[type]/[id]/page.tsx` pass the full `ContentTypeConfig` — including the Zod `schema` (a class instance) — from a Server Component into the client `ContentForm`. Zod schemas are not RSC-serializable, so Next throws `Only plain objects … can be passed to Client Components` on a render timer and the response stream never closes (request hangs). Fix: strip `schema` (and any non-plain fields) from `config` before passing to `ContentForm`, or pass only a serializable field/column subset. Shipped in `f28a54d` (carousel); affects carousel + club. Blocks the create happy-path in `tests/e2e/club-admin.spec.ts`.
- **[MED] Admin e2e login selector is wrong.** `tests/e2e/carousel-admin.spec.ts` `beforeEach` uses `input[name="email"]` / `input[name="password"]`, but `app/admin/login/page.tsx` inputs have no `name` (controlled, type-based). The carousel spec fails at login and never exercised the create hang above. Switch to `input[type="email"]` / `input[type="password"]` (as done in `club-admin.spec.ts`).
