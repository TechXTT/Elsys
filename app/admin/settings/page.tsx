...
export default function SettingsPage() {
  ...
  async function onChangePassword(e: React.FormEvent) {
    ...
    setError(null);
    try {
      if (!currentPassword || !newPassword) throw new Error(t("errEnterPw"));
      if (newPassword !== confirmPassword) throw new Error(t("errMismatch"));
      const res = await fetch("/api/admin/me", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || t("errChange"));
      setPassMessage(t("passwordChanged"));
      setCurrentPassword("");
      setNewPassword("");
      setConfirmPassword("");
    } catch (e: any) {
      setError(e.message || t("errChange"));
    } finally {
      ...
      {loading ? (
        ...
        <>
          <section className="rounded border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-slate-100">{t("profile")}</h2>
            <form className="grid grid-cols-1 gap-3 sm:grid-cols-2" onSubmit={onSaveProfile}>
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-slate-700 dark:text-slate-200">{t("email")}</span>
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-slate-700 dark:text-slate-200">{t("firstName")}</span>
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-slate-700 dark:text-slate-200">{t("lastName")}</span>
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-slate-700 dark:text-slate-200">{t("grade")}</span>
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-slate-700 dark:text-slate-200">{t("classLabel")}</span>
              ...
              <div className="sm:col-span-2">
                <button disabled={saving} className="rounded bg-blue-600 px-3 py-1 text-sm font-medium text-white disabled:opacity-60">{t("save")}</button>
          ...
          <section className="rounded border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-slate-100">{t("changePassword")}</h2>
            <form className="grid grid-cols-1 gap-3 sm:grid-cols-2" onSubmit={onChangePassword}>
              <label className="flex flex-col gap-1 text-sm sm:col-span-2">
                <span className="text-slate-700 dark:text-slate-200">{t("currentPassword")}</span>
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-slate-700 dark:text-slate-200">{t("newPassword")}</span>
              ...
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-slate-700 dark:text-slate-200">{t("confirmPassword")}</span>
              ...
              <div className="sm:col-span-2">
                <button disabled={changingPass} className="rounded bg-blue-600 px-3 py-1 text-sm font-medium text-white disabled:opacity-60">{t("updatePassword")}</button>
